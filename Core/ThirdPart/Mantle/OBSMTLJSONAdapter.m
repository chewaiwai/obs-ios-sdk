...
NSString  *const OBSMTLJSONAdapterErrorDomain = @"OBSMTLJSONAdapterErrorDomain";
const NSInteger OBSMTLJSONAdapterErrorNoClassFound = 2;
const NSInteger OBSMTLJSONAdapterErrorInvalidJSONDictionary = 3;
const NSInteger OBSMTLJSONAdapterErrorInvalidJSONMapping = 4;

// An exception was thrown and caught.
const NSInteger OBSMTLJSONAdapterErrorExceptionThrown = 1;

// Associated with the NSException that was caught.
NSString  *const OBSMTLJSONAdapterThrownExceptionErrorKey = @"OBSMTLJSONAdapterThrownException";

@interface OBSMTLJSONAdapter ()

// The OBSMTLModel subclass being parsed, or the class of `model` if parsing has
// completed.
...
//              to <OBSMTLJSONSerializing>. This argument must not be nil.
...
@implementation OBSMTLJSONAdapter

#pragma mark Convenience methods

+ (id)modelOfClass:(Class)modelClass fromJSONDictionary:(NSDictionary *)JSONDictionary error:(NSError **)error {
	OBSMTLJSONAdapter *adapter = [[self alloc] initWithModelClass:modelClass];
...
+ (NSArray *)modelsOfClass:(Class)modelClass fromJSONArray:(NSArray *)JSONArray error:(NSError **)error {
	if (JSONArray == nil || ![JSONArray isKindOfClass:NSArray.class]) {
		if (error != NULL) {
			...
			*error = [NSError errorWithDomain:OBSMTLJSONAdapterErrorDomain code:OBSMTLJSONAdapterErrorInvalidJSONDictionary userInfo:userInfo];
		}
		return nil;
	}
	...
	for (NSDictionary *JSONDictionary in JSONArray){
		OBSMTLModel *model = [self modelOfClass:modelClass fromJSONDictionary:JSONDictionary error:error];
...
+ (NSDictionary *)JSONDictionaryFromModel:(id<OBSMTLJSONSerializing>)model error:(NSError **)error {
	OBSMTLJSONAdapter *adapter = [[self alloc] initWithModelClass:model.class];
...
+ (NSArray *)JSONArrayFromModels:(NSArray *)models error:(NSError **)error {
	...
	for (OBSMTLModel<OBSMTLJSONSerializing> *model in models) {
...
+ (NSDictionary *)valueTransformersForModelClass:(Class)modelClass {
	...
	for (NSString *key in [modelClass propertyKeys]) {
		...
		if (*(attributes->type) == *(@encode(id))) {
			...
			if (transformer == nil) transformer = [NSValueTransformer obs_mtl_validatingTransformerForClass:propertyClass ?: NSObject.class];
		} else {
...
- (OBSMTLJSONAdapter *)JSONAdapterForModelClass:(Class)modelClass error:(NSError **)error {
	...
	@synchronized(self) {
		OBSMTLJSONAdapter *result = [self.JSONAdaptersByModelClass objectForKey:modelClass];
...
+ (NSValueTransformer *)transformerForModelPropertiesOfClass:(Class)modelClass {
	...
	SEL selector = OBSMTLSelectorWithKeyPattern(NSStringFromClass(modelClass), "JSONTransformer");
...
+ (NSValueTransformer *)transformerForModelPropertiesOfObjCType:(const char *)objCType {
	...
	if (strcmp(objCType, @encode(BOOL)) == 0) {
		return [NSValueTransformer valueTransformerForName:OBSMTLBooleanValueTransformerName];
	}
...
@implementation OBSMTLJSONAdapter (ValueTransformers)
...
+ (NSValueTransformer *)NSURLJSONTransformer {
	return [NSValueTransformer valueTransformerForName:OBSMTLURLValueTransformerName];
}

+ (NSValueTransformer *)NSUUIDJSONTransformer {
	return [NSValueTransformer valueTransformerForName:OBSMTLUUIDValueTransformerName];
}

@end

@implementation OBSMTLJSONAdapter (Deprecated)
...
+ (NSDictionary *)JSONDictionaryFromModel:(OBSMTLModel<OBSMTLJSONSerializing> *)model {
