...
static NSString  *const OBSMTLModelVersionKey = @"OBSMTLModelVersion";

// Used to cache the reflection performed in +allowedSecureCodingClassesByPropertyKey.
static void *OBSMTLModelCachedAllowedClassesKey = &OBSMTLModelCachedAllowedClassesKey;
...
static NSSet *encodablePropertyKeysForClass(Class modelClass) {
	return [[modelClass encodingBehaviorsByPropertyKey] keysOfEntriesPassingTest:^ BOOL (NSString *propertyKey, NSNumber *behavior, BOOL *stop) {
		return behavior.unsignedIntegerValue != OBSMTLModelEncodingBehaviorExcluded;
	}];
}
...
@implementation OBSMTLModel (NSCoding)
...
+ (NSDictionary *)encodingBehaviorsByPropertyKey {
	...
	for (NSString *key in propertyKeys) {
		...
		OBSMTLModelEncodingBehavior behavior = (attributes->weak ? OBSMTLModelEncodingBehaviorConditional : OBSMTLModelEncodingBehaviorUnconditional);
...
+ (NSDictionary *)allowedSecureCodingClassesByPropertyKey {
	NSDictionary *cachedClasses = objc_getAssociatedObject(self, OBSMTLModelCachedAllowedClassesKey);
	...
	objc_setAssociatedObject(self, OBSMTLModelCachedAllowedClassesKey, allowedClasses, OBJC_ASSOCIATION_COPY);
...
- (id)decodeValueForKey:(NSString *)key withCoder:(NSCoder *)coder modelVersion:(NSUInteger)modelVersion {
	...
	SEL selector = OBSMTLSelectorWithCapitalizedKeyPattern("decode", key, "WithCoder:modelVersion:");
...
- (instancetype)initWithCoder:(NSCoder *)coder {
	...
	if (requiresSecureCoding) {
		version = [coder decodeObjectOfClass:NSNumber.class forKey:OBSMTLModelVersionKey];
	} else {
		version = [coder decodeObjectForKey:OBSMTLModelVersionKey];
	}
...
- (void)encodeWithCoder:(NSCoder *)coder {
	...
	[coder encodeObject:@(self.class.modelVersion) forKey:OBSMTLModelVersionKey];
	...
	[self.dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
		@try {
			...
			switch ([encodingBehaviors[key] unsignedIntegerValue]) {
				...
				case OBSMTLModelEncodingBehaviorExcluded:
					break;
					
				case OBSMTLModelEncodingBehaviorUnconditional:
					[coder encodeObject:value forKey:key];
					break;
					
				case OBSMTLModelEncodingBehaviorConditional:
...
@implementation OBSMTLModel (OldArchiveSupport)
