...
static void *OBSMTLModelCachedPropertyKeysKey = &OBSMTLModelCachedPropertyKeysKey;

// Associated in +generateAndCachePropertyKeys with a set of all transitory
// property keys.
static void *OBSMTLModelCachedTransitoryPropertyKeysKey = &OBSMTLModelCachedTransitoryPropertyKeysKey;

// Associated in +generateAndCachePropertyKeys with a set of all permanent
// property keys.
static void *OBSMTLModelCachedPermanentPropertyKeysKey = &OBSMTLModelCachedPermanentPropertyKeysKey;
...
// occurred.
static BOOL OBSMTLValidateAndSetValue(id obj, NSString *key, id value, BOOL forceUpdate, NSError **error) {
	...
	@try {
		...
		#else
		if (error != NULL) {
			*error = [NSError obs_mtl_modelErrorWithException:ex];
		}
...
// +storageBehaviorForPropertyWithKey returned OBSMTLPropertyStorageTransitory.
+ (NSSet *)transitoryPropertyKeys;

// Returns a set of all property keys for which
// +storageBehaviorForPropertyWithKey returned OBSMTLPropertyStoragePermanent.
+ (NSSet *)permanentPropertyKeys;

// Enumerates all properties of the receiver's class hierarchy, starting at the
// receiver, and continuing up until (but not including) OBSMTLModel.
//
...
+ (void)generateAndCacheStorageBehaviors {
	...
	for (NSString *propertyKey in self.propertyKeys) {
		switch ([self storageBehaviorForPropertyWithKey:propertyKey]) {
			case OBSMTLPropertyStorageNone:
				break;

			case OBSMTLPropertyStorageTransitory:
				[transitoryKeys addObject:propertyKey];
				break;

			case OBSMTLPropertyStoragePermanent:
	...
	objc_setAssociatedObject(self, OBSMTLModelCachedTransitoryPropertyKeysKey, transitoryKeys, OBJC_ASSOCIATION_COPY);
	objc_setAssociatedObject(self, OBSMTLModelCachedPermanentPropertyKeysKey, permanentKeys, OBJC_ASSOCIATION_COPY);
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary error:(NSError **)error {
	...
	for (NSString *key in dictionary) {
		...
		BOOL success = OBSMTLValidateAndSetValue(self, key, value, YES, error);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
	...
	while (!stop && ![cls isEqual:OBSMTLModel.class]) {
		unsigned count = 0;
...
+ (NSSet *)propertyKeys {
	NSSet *cachedKeys = objc_getAssociatedObject(self, OBSMTLModelCachedPropertyKeysKey);
	...
	[self enumeratePropertiesUsingBlock:^(objc_property_t property, BOOL *stop) {
		...
		if ([self storageBehaviorForPropertyWithKey:key] != OBSMTLPropertyStorageNone) {
			 [keys addObject:key];
		}
	}];
	...
	// It doesn't really matter if we replace another thread's work, since we do
	// it atomically and the result should be the same.
	objc_setAssociatedObject(self, OBSMTLModelCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);

	return keys;
}

+ (NSSet *)transitoryPropertyKeys {
	NSSet *transitoryPropertyKeys = objc_getAssociatedObject(self, OBSMTLModelCachedTransitoryPropertyKeysKey);

	if (transitoryPropertyKeys == nil) {
		[self generateAndCacheStorageBehaviors];
		transitoryPropertyKeys = objc_getAssociatedObject(self, OBSMTLModelCachedTransitoryPropertyKeysKey);
	}
...
+ (NSSet *)permanentPropertyKeys {
	NSSet *permanentPropertyKeys = objc_getAssociatedObject(self, OBSMTLModelCachedPermanentPropertyKeysKey);

	if (permanentPropertyKeys == nil) {
		[self generateAndCacheStorageBehaviors];
		permanentPropertyKeys = objc_getAssociatedObject(self, OBSMTLModelCachedPermanentPropertyKeysKey);
	}
...
+ (OBSMTLPropertyStorage)storageBehaviorForPropertyWithKey:(NSString *)propertyKey {
	...
	if (property == NULL) return OBSMTLPropertyStorageNone;
	...
	if (!attributes->dynamic && attributes->ivar == NULL && !hasGetter && !hasSetter) {
		return OBSMTLPropertyStorageNone;
	} else if (attributes->readonly && attributes->ivar == NULL) {
		if ([self isEqual:OBSMTLModel.class]) {
			return OBSMTLPropertyStorageNone;
		} else {
		...
		return OBSMTLPropertyStoragePermanent;
	}
}

#pragma mark Merging

- (void)mergeValueForKey:(NSString *)key fromModel:(NSObject<OBSMTLModel> *)model {
	NSParameterAssert(key != nil);

	SEL selector = OBSMTLSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
	...
	void (*function)(id, SEL, id<OBSMTLModel>) = (__typeof__(function))imp;
...
- (BOOL)validate:(NSError **)error {
	for (NSString *key in self.class.propertyKeys) {
		...
		BOOL success = OBSMTLValidateAndSetValue(self, key, value, NO, error);
...
- (instancetype)copyWithZone:(NSZone *)zone {
	OBSMTLModel *copy = [[self.class allocWithZone:zone] init];
...
- (BOOL)isEqual:(OBSMTLModel *)model {
