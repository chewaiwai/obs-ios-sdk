...
obs_mtl_propertyAttributes *obs_mtl_copyPropertyAttributes (objc_property_t property) {
    ...
    obs_mtl_propertyAttributes *attributes = calloc(1, sizeof(obs_mtl_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate obs_mtl_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = obs_mtl_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = obs_mtl_propertyMemoryManagementPolicyRetain;
            break;
